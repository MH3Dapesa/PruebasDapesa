...
{
    public partial class Bancomer : Form
    {
        public Bancomer()
        {
            InitializeComponent();
            ...
            ofdArchivo.Filter = "files Default (*.exp)|*.exp";
        ...
        private void Procesar(string loRutaArchivoMovimientos, int lnIniciarLecturaMovimientos, string loRutaArchivoCodigosMovimientos)
        {
            ...
            try
            {
                ...
                if (poMovimientos.Rows.Count < 1)
                {
                    MessageBox.Show("¡No hay registros en el documento actual con el rango de fecha especificado, verificar!", "Mensaje", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }
                ...
                this.gvMovimientosCorrectos.Columns["Columna2"].Visible = false;
                ...
                this.gvMovimientosErrores.Columns["Columna2"].Visible = false;
            ...
            try
            {
                ...
                for (int lnFila = 0; lnFila < poMovimientos.Rows.Count; lnFila++)
                {
                    DataTable loTablaFiltrada = (DataTable)loMovimientos.ValidaCobro(((InicioSesion)this.MdiParent.Owner).Sesion,
                        poMovimientos.Rows[lnFila]["Guia_CIE"].ToString(), Convert.ToDateTime(poMovimientos.Rows[lnFila]["Fecha"].ToString()));
                    ...
                    if (loTablaFiltrada.Rows.Count > 0)
                    {
                        for (int i = 0; i < loTablaFiltrada.Rows.Count; i++)
                        {
                            ...
                            foreach (DataRow loFila in poMovimientos.Rows)
                            {
                                ...
                                {
                                    if (loFila["Guia_CIE"].ToString() == loTablaFiltrada.Rows[i]["NUM_TRANSACCION"].ToString())
                                    {
                ...
                for (int i = poMovimientos.Rows.Count - 1; i >= 0; i--)
                {
                    ...
                    for (int l = 0; l < loResultado.Rows.Count; l++)
                    {
                        ...
                        if (loFila["Guia_CIE"].ToString() == loFilaCompara["Guia_CIE"].ToString())
                        {
                            loFila.Delete();
                            break;
                        }
                ...
                this.gvExistentes.Columns["Columna2"].Visible = false;
        ...
        public void InsertaCobrosSIIL()
        {
            try
            {
                ...
                foreach (DataGridViewRow loFila in gvMovimientosCorrectos.Rows)
                {
                    ...
                    if (Convert.ToBoolean(cbVerdadero.Value))
                    {
                        ...
                        if (loMovimientos.InsertarCobroSIIL(
                            ...
                            , "BANCOMER", int.Parse(loFila.Cells["Forma_pago"].Value.ToString()), loFila.Cells["Cliente"].Value.ToString(), int.Parse(loFila.Cells["Suc"].Value.ToString())))
                        { }
                    }
                    pbMovimientos.PerformStep();
                }
                ...
                if (loMovimientos.InsertaCobroBancomerTablaDAP(
                            ((InicioSesion)this.MdiParent.Owner).Sesion, loTablaCorrectos, psArchivo, 4) // 4 - indica cuenta bancaria "Bancomer Concentradora"
                            )
                { }
        ...
        {
            poMovimientos.Columns["Columna1"].ColumnName = "Fecha";
            poMovimientos.Columns["Columna3"].ColumnName = "Guia_CIE";
            poMovimientos.Columns["Columna4"].ColumnName = "Referencia";
            poMovimientos.Columns["Columna5"].ColumnName = "Concepto";
            poMovimientos.Columns["Columna6"].ColumnName = "Importe";
            return poMovimientos;
        }
        ...
        private DataTable obtenerFormaDePago(DataTable poMovimientos)
        {
            ...
            for (int lnFila = 0; lnFila < poMovimientos.Rows.Count; lnFila++)
            {
                DataTable loResultado = loMovimientos.ObtenerFormaDePago(((InicioSesion)this.MdiParent.Owner).Sesion, poMovimientos.Rows[lnFila][1].ToString());
        ...
        private DataTable ObtenerSucursalRegistrosProcesados(DataTable poMovimientos)
        {
            ...
            for (int lnFila = 0; lnFila < poMovimientos.Rows.Count; lnFila++)
            {
                DataTable loResultado = loSucursal.ObtenerSucursal(((InicioSesion)this.MdiParent.Owner).Sesion, poMovimientos.Rows[lnFila]["Referencia"].ToString());
        ...
        private DataTable LeerArchivo(string loRutaArchivo, int lnRow)
        {
            try
            {
                ...
                for (int i = 0; i < ((loEncabezados.Length - 1)) - 1; i++)
                {
                    loFila = loDatos.NewRow();
                    loFila["Columna1"] = loEncabezados[i];
                    loFila["Columna2"] = loEncabezados[i + 1];
                    loFila["Columna3"] = loEncabezados[i + 2];
                    loFila["Columna4"] = loEncabezados[i + 3];
                    loFila["Columna5"] = loEncabezados[i + 4];
                    loFila["Columna6"] = loEncabezados[i + 5];
                ...
                for (int i = loDatos.Rows.Count - 1; i >= 0; i--)
                {
                    ...
                    if (loFilasFecha[0].ToString().Length < 6)
                    {
                        string sCadena = loFilasFecha[0].ToString().Substring(0, 5);
                        sCadena = sCadena.PadLeft(6, '0');
                        loFilasFecha[0] = sCadena;
                    }
                }
                ...
                {
                    DataRow loFilasFecha = loDatos.Rows[i];
                    string sAño = loFilasFecha[0].ToString().Substring(4, 2);
                ...
                DataTable loDatosFiltrados = loDatos.Clone();

                for (int i = loDatos.Rows.Count - 1; i >= 0; i--)
                {
                    DataRow loFilasFecha = loDatos.Rows[i];
                    DateTime dtFecha = DateTime.Parse(loFilasFecha[0].ToString().Substring(0, 8));
                    DateTime dtFechaInicia = DateTime.Parse(dtpFechaInicial.Text);
                    DateTime dtFechaFinal = DateTime.Parse(dtpFechaFinal.Text);
                    if ((dtFecha >= dtFechaInicia) && (dtFecha <= dtFechaFinal))
                    {
                        loDatosFiltrados.ImportRow(loFilasFecha);
                    }
                }

                return loDatosFiltrados;
        ...
        {
            MovimientosBBVA loInforme = new MovimientosBBVA();
        ...
        private void btnProcesar_Click(object sender, EventArgs e)
        {
            ...
            {
                Procesar(ofdArchivo.FileName, 0, Path.Combine(Application.StartupPath, @"Documentos\Codigos_Banamex.csv"));
            }
        }

        private void btnImprimirMovimientoC_Click(object sender, EventArgs e)
        {
            Imprimir((DataTable)gvMovimientosCorrectos.DataSource, "MOVIMIENTOS BBVA - CORRECTO");
        }

        private void btnExportarXlsxC_Click(object sender, EventArgs e)
        {
            Guardar((DataTable)gvMovimientosCorrectos.DataSource, "MOVIMIENTOS BBVA - CORRECTO", (int)Comun.Definiciones.TipoArchivo.EXCEL);
        }

        private void btnGuardarMovimientoC_Click(object sender, EventArgs e)
        {
            Guardar((DataTable)gvMovimientosCorrectos.DataSource, "MOVIMIENTOS BBVA - CORRECTO", (int)Comun.Definiciones.TipoArchivo.PDF);
        }

        private void btnImprimirMovimientoR_Click(object sender, EventArgs e)
        {
            Imprimir((DataTable)gvMovimientosErrores.DataSource, "MOVIMIENTOS BBVA - REVISAR");
        }

        private void btnExportarXlsxR_Click(object sender, EventArgs e)
        {
            Guardar((DataTable)gvMovimientosErrores.DataSource, "MOVIMIENTOS BBVA - REVISAR", (int)Comun.Definiciones.TipoArchivo.EXCEL);
        }

        private void btnGuardarMovimientoR_Click(object sender, EventArgs e)
        {
            Guardar((DataTable)gvMovimientosErrores.DataSource, "MOVIMIENTOS BBVA - REVISAR", (int)Comun.Definiciones.TipoArchivo.PDF);
        }
